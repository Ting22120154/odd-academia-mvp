          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-[var(--shadow-md)]">
            {reportStatus === "done" ? (
              <div className="text-center">
                <p className="text-sm font-semibold text-zinc-900">Report submitted</p>
                <p className="mt-1 text-xs text-zinc-500">Thanks — our team will review it shortly.</p>
                <button
                  type="button"
                  onClick={() => { setReportingUser(false); setReportStatus("idle"); setReportSubject(""); setReportReason(""); }}
                  className="mt-4 rounded-lg bg-zinc-900 px-4 py-2 text-xs font-semibold text-white hover:opacity-90"
                >
                  Close
                </button>
              </div>
              ...
              <>
                <h2 className="text-sm font-semibold text-zinc-900">Report this user</h2>
                <div className="mt-4 space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-black/[0.08] px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-700">Details</label>
                    <textarea
                      value={reportReason}
                ...
                <div className="mt-4 flex justify-end gap-2">
                  <button
                    type="button"
                    onClick={() => { setReportingUser(false); setReportSubject(""); setReportReason(""); }}
                    className="rounded-lg border border-black/[0.08] px-4 py-2 text-xs font-semibold text-zinc-600 hover:bg-zinc-50"
                  >
                    Cancel
                  </button>
                  <button
                    type="button"
                    disabled={reportStatus === "sending" || !reportSubject.trim() || !reportReason.trim()}
                    onClick={async () => {
                      ...
                      const res = await fetch("/api/reports", {
                        ...
                        credentials: "include",
                        body:        JSON.stringify({ type: "user", reportedId: id, subject: reportSubject.trim(), reason: reportReason.trim() }),
                      });
                      setReportStatus(res.ok ? "done" : "idle");
                    }}
                    className="rounded-lg bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:opacity-90 disabled:opacity-50"
                  >
