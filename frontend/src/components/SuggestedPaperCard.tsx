...
export function SuggestedPaperCard({ post }: Props) {
  ...
  return (
    <li className="group relative list-none">
      <Link
        href={`/posts/${post.id}`}
        className="flex h-full flex-col overflow-hidden rounded-2xl border border-black/[0.06] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] transition-[transform,box-shadow] hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] dark:border-white/10 dark:bg-zinc-950"
      >
        ...
        <div className="flex flex-1 flex-col gap-3 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {tags.slice(0, 2).map((t) => (
              <span
                key={t}
                className="inline-flex items-center rounded-full border border-black/[0.06] bg-zinc-50 px-2.5 py-0.5 text-xs font-medium text-zinc-700 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-200"
              >
                {t}
              </span>
            ))}
        ...
        <span
          className="pointer-events-none absolute bottom-0 right-0 h-0 w-0 border-b-[14px] border-l-[14px] border-b-sky-200/90 border-l-transparent dark:border-b-sky-500/40"
          aria-hidden
        />
      </Link>
    </li>
